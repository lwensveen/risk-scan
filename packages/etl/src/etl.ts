...
import { riskScanConfig } from '@risk-scan/utils';
...
export async function ingestSnapshots() {
  for (const t of riskScanConfig.officeReitTickers) {
  ...
  for (const t of riskScanConfig.healthcareTickers) {
  ...
  for (const t of riskScanConfig.regionalBankTickers) {
